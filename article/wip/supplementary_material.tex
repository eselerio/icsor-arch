...
\newcommand{\pending}{\textit{Pending final CUDA benchmark}}
...
\draftnotice{The full CUDA benchmark has not yet been executed. All result cells and figure panels in this file therefore remain explicit placeholders. No numerical performance result should be inferred from this planned structure.}
...
The model set comprises XGBoost, LightGBM, CatBoost, AdaBoost, Random Forest, support vector regression, $k$-nearest neighbors, partial least squares, a multilayer perceptron, and TabICLv2. The nine conventional methods use fold-local selection and preprocessing as specified in the main manuscript. TabICLv2 uses the fixed zero-shot configuration in Table~\ref{tab:s_tabiclv2_configuration}.

\begin{table}[htbp]
\centering
\caption{Fixed TabICLv2 configuration to be retained in every fold, data size, timing run, and final full-context construction for OOD scoring.}
\label{tab:s_tabiclv2_configuration}
\small
\begin{tabular}{p{4.2cm}p{9.0cm}}
\toprule
Item & Fixed setting \\
\midrule
Interface & \texttt{TabICLRegressor}, package version 2.1.1 \\
Checkpoint & \texttt{tabicl-regressor-v2-20260212.ckpt}; 114,324,594 bytes; SHA-256 \texttt{0db9cb538f114e79026bf08f45f41ad8}\newline\texttt{dd7ad2de2aaca9a5ca8cd3bd9748ae7a} \\
Adaptation & Zero-shot; no checkpoint fine-tuning \\
Target handling & One independent context for each of the 20 component targets \\
Context rows & All applicable training rows; no row subsampling and no context-size cap \\
Ensemble & Eight estimators \\
Normalization & \texttt{none} and \texttt{power} \\
Feature shuffle & Latin \\
Prediction calls and internal batch & Each complete evaluation partition in one call per component; internal batch size 8 \\
Key--value cache & Disabled \\
Device and precision & Explicit \texttt{cuda:0}, float32; GPU-local offloading \\
Acceleration modes & Automatic mixed precision disabled; FlashAttention~3 disabled; automatic checkpoint download disabled \\
Repeatability & Seed 42; repeated predictions must agree within $10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}
...
For each model--size cell, a following summary row will report the descriptive five-fold mean $\pm$ sample SD. The fold rows will remain available so that the summary can be checked directly. Preparation time means preprocessing plus conventional fitting for fitted learners and context preparation for TabICLv2; these operations will not be combined without an explicit label.
...
The OOD supplement will preserve mild and severe extrapolation as separate strata and will report candidate attempts, solver failures, accepted states, and acceptance rates before any surrogate comparison. No OOD response will enter fitting, scaling, context construction, calibration, or model selection. Conventional methods will be refitted on all 10,000 in-distribution states with settings frozen before OOD scoring. TabICLv2 will construct one full 10,000-row context for each of the 20 targets with the fixed configuration in Table~\ref{tab:s_tabiclv2_configuration}.
...
The timing set contains 512 validation rows, constructed deterministically by cycling through the applicable validation inputs. Five warm-up calls precede 30 synchronized repetitions for each timed path. Timing is partitioned into preprocessing and fitting or context preparation, raw inference, projection, and end-to-end projected inference. The median 512-row latency within each fold is divided by 512; the five fold medians are then reported individually and as descriptive mean $\pm$ sample SD in milliseconds per sample. CUDA synchronization brackets accelerator measurements. TabICLv2 retains internal batch size 8, \texttt{cuda:0} float32, GPU-local offloading, disabled automatic mixed precision, disabled FlashAttention~3, and disabled key--value caching during timing. Hyperparameter-search time is reported separately.
...
The scientific provenance record will identify the benchmark data design, fold design, random seed, package and library versions, checkpoint identifier, hardware, operating system, thread limits, and numeric precision used for the accepted analysis.
...
TabICLv2 package and interface & \texttt{TabICLRegressor} 2.1.1 \\
TabICLv2 checkpoint identifier & \texttt{tabicl-regressor-v2-20260212.ckpt} \\
Processor, CUDA accelerator, and memory & \pending \\
...
{Replace with final nRMSE curves for all ten models at the eleven nested data sizes. Preserve the same fold assignments, model colors, and metric label used in the main manuscript.}
...
\begin{enumerate}
\item every displayed performance value comes from the completed final CUDA benchmark;
...
\item the TabICLv2 execution record matches Table~\ref{tab:s_tabiclv2_configuration};
