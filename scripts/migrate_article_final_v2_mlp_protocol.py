"""Apply the authorized, MLP-only protocol amendment to article_final_v2.

The migration preserves every completed non-MLP artifact, archives the
exploratory MLP tuning study outside the accepted run root, and updates the
resolved configuration and manifest contract. It is intentionally one-shot
and refuses any unexpected model or artifact change.
"""
...
OLD_CONFIG_SHA256 = "8d9f44d7a057ab886cd1617ffff8f9873941b3d4519fc130354dff70515ebde7"
EXPECTED_NOTEBOOK_SHA256 = "b7471b8c458b3af9ceca050fedbbb732869c8d2168e98caba9991db5a37856b2"
EXPECTED_MLP_FILES = {"outer_0.sampler.pkl", "outer_0.sqlite3", "outer_0.timing.json"}
...
def main() -> None:
    ...
    if changed_models != ["ann_deep_regressor"]:
        raise RuntimeError(f"The migration is not MLP-only: {changed_models}")

    mlp_dir = RUN_ROOT / "tuning" / "ann_deep_regressor"
    require_within(mlp_dir, RUN_ROOT)
    observed = {path.name for path in mlp_dir.iterdir()} if mlp_dir.is_dir() else set()
    if observed != EXPECTED_MLP_FILES:
        raise RuntimeError(f"Unexpected exploratory MLP artifact set: {sorted(observed)}")
    unexpected_mlp = [
        path.relative_to(RUN_ROOT).as_posix()
        for path in RUN_ROOT.rglob("*")
        if path.is_file()
        and ("ann_deep_regressor" in path.name or "ann_deep_regressor" in path.parts)
        and mlp_dir not in path.parents
    ]
    if unexpected_mlp:
        raise RuntimeError(f"Unexpected MLP-derived artifacts exist: {unexpected_mlp}")

    timestamp = datetime.now(timezone.utc)
    backup_dir = BACKUP_ROOT / f"mlp_protocol_migration_{timestamp.strftime('%Y%m%d_%H%M%S')}"
    ...
    archived_mlp = backup_dir / "invalidated_ann_deep_regressor"
    shutil.move(str(mlp_dir), str(archived_mlp))
    ...
    amendment = {
        ...
        "scope": "ann_deep_regressor_only",
        "basis": "computational_feasibility_before_retaining_any_MLP_outer_fold_result",
        "previous_config_sha256": OLD_CONFIG_SHA256,
        "accepted_config_sha256": new_config_sha256,
        "invalidated_artifacts": sorted(EXPECTED_MLP_FILES),
        "archived_outside_run_root": archived_mlp.relative_to(ROOT).as_posix(),
        "previous_mlp_protocol": old_models["ann_deep_regressor"],
        "accepted_mlp_protocol": new_models["ann_deep_regressor"],
    ...
    if mlp_dir.exists():
        raise RuntimeError("The invalidated MLP directory remains in the run root.")
        ...
        json.dumps(
            {
                ...
                "invalidated_mlp_files": sorted(EXPECTED_MLP_FILES),
            },
            indent=2,
        )
    )
