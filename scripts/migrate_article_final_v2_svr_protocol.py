"""Apply the authorized, SVR-only protocol amendment to article_final_v2.

This one-time migration preserves completed artifacts from model families whose
configuration did not change, archives the exploratory SVR study outside the
run root, and updates the run's immutable configuration snapshot and manifest
contract.  It refuses unexpected source contracts or SVR artifacts.
"""
...
OLD_CONFIG_SHA256 = "dabce4d639b092817a76b8f92896f26f4affff727d92712d19c06daf07d8f289"
EXPECTED_NOTEBOOK_SHA256 = "b7471b8c458b3af9ceca050fedbbb732869c8d2168e98caba9991db5a37856b2"
EXPECTED_SVR_FILES = {"outer_0.sampler.pkl", "outer_0.sqlite3", "outer_0.timing.json"}
...
def main() -> None:
    ...
    if changed_models != ["svr_regressor"]:
        raise RuntimeError(f"The migration is not SVR-only: {changed_models}")

    svr_dir = RUN_ROOT / "tuning" / "svr_regressor"
    require_within(svr_dir, RUN_ROOT)
    observed = {path.name for path in svr_dir.iterdir()} if svr_dir.is_dir() else set()
    if observed != EXPECTED_SVR_FILES:
        raise RuntimeError(f"Unexpected exploratory SVR artifact set: {sorted(observed)}")
    unexpected_svr = [
        path.relative_to(RUN_ROOT).as_posix()
        for path in RUN_ROOT.rglob("*")
        if path.is_file()
        and ("svr_regressor" in path.name or "svr_regressor" in path.parts)
        and svr_dir not in path.parents
    ]
    if unexpected_svr:
        raise RuntimeError(f"Unexpected SVR-derived artifacts exist: {unexpected_svr}")

    timestamp = datetime.now(timezone.utc)
    backup_dir = BACKUP_ROOT / f"svr_protocol_migration_{timestamp.strftime('%Y%m%d_%H%M%S')}"
    ...
    archived_svr = backup_dir / "invalidated_svr_regressor"
    shutil.move(str(svr_dir), str(archived_svr))
    ...
    amendment = {
        ...
        "scope": "svr_regressor_only",
        "basis": "computational_feasibility_before_retaining_any_SVR_result",
        "previous_config_sha256": OLD_CONFIG_SHA256,
        "accepted_config_sha256": new_config_sha256,
        "invalidated_artifacts": sorted(EXPECTED_SVR_FILES),
        "archived_outside_run_root": archived_svr.relative_to(ROOT).as_posix(),
        "previous_svr_protocol": old_models["svr_regressor"],
        "accepted_svr_protocol": new_models["svr_regressor"],
    ...
    if svr_dir.exists():
        raise RuntimeError("The invalidated SVR directory remains in the run root.")
        ...
        json.dumps(
            {
                ...
                "invalidated_svr_files": sorted(EXPECTED_SVR_FILES),
            },
            indent=2,
        )
    )
